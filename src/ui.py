    ...
    while True:
        try:
            ...
            if 1 <= choice <= len(choices):
                ...
                print_warn("Invalid choice. Please enter a number from the list.")
        except ValueError:
...
class TestReporter:
    ...
    def generate_summary(
        self, test_title: str, checks_config: dict, multiple_domains: bool, repeats: int
    ) -> str:
    ...
    def print_and_save(
        ...
        console = "\n" + console_body.replace("# ", Style.BRIGHT + Fore.GREEN).replace(
            "strategies", f"strategies{Style.RESET_ALL}"
        )
        print(console)
        ...
        if dns_stats:
            ...
            if total > 0:
                ...
                print_info(
                    f"DNS Cache Stats: {dns_stats['hits']} hits, {dns_stats['misses']} misses ({hit_rate:.1f}% hit rate)"
                )
